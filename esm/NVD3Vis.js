...
import { cleanColorInput, computeBarChartWidth, computeYDomain, computeStackedYDomain, drawBarValues, generateBubbleTooltipContent, generateMultiLineTooltipContent, generateRichLineTooltipContent, generateTimePivotTooltip, generateTooltipClassName, generateAreaChartTooltipContent, getMaxLabelSize, getTimeOrNumberFormatter, hideTooltips, tipFactory, tryNumify, removeTooltip, setAxisShowMaxMin, stringifyTimeRange, wrapTooltip } from './utils';
  ...
  const drawGraph = function drawGraph() {
    ...
    if (isVizTypes(['line', 'area']) && useRichTooltip) {
      ...
      if (vizType === 'line') {
        ...
        // area chart
        chart.interactiveLayer.tooltip.contentGenerator(d => generateAreaChartTooltipContent(d, smartDateVerboseFormatter, yAxisFormatter));
      }
    }
      ...
      if (isTimeSeries && activeAnnotationLayers.length > 0) {
        ...
        if (annotationData) {
          ...
          activeAnnotationLayers.filter(x => x.annotationType === ANNOTATION_TYPES.EVENT && annotationData && annotationData[x.name]).forEach((config, index) => {
            ...
            const tip = tipFactory(_extends({}, e, {
