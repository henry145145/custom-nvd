...
import { smartDateFormatter } from '@superset-ui/time-format'; // Regexp for the label added to time shifted series
...
export function tipFactory(layer) {
  return d3tip().attr('class', "d3-tip " + (layer.annotationTipClass || '')).direction('n').offset([-5, 0]).html(d => {
    if (!d) {
      return '';
    }
