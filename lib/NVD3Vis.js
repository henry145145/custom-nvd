...
function nvd3Vis(element, props) {
  ...
  const drawGraph = function drawGraph() {
    ...
    if (chart.yAxis !== undefined || chart.yAxis2 !== undefined) {
      ...
      svg.datum(data).transition().duration(500).attr('width', width).attr('height', height).attr('transform', "translate(" + width + ",0)").call(chart); // On scroll, hide (not remove) tooltips so they can reappear on hover.
