...
_nvd.default.utils.noData = function (chart, container) {
  const opt = chart.options();
  const margin = opt.margin();

  const height = _nvd.default.utils.availableHeight(null, container, margin);

  const width = _nvd.default.utils.availableWidth(null, container, margin);
...
const {
  getColor,
  getScale
} = _color.CategoricalColorNamespace; // Limit on how large axes margins can grow as the chart window is resized
...
const propTypes = {
  ...
  pieLabelType: _propTypes.default.oneOf(['key', 'value', 'percent', 'key_value', 'key_percent']),
...
function nvd3Vis(element, props) {
  ...
  const drawGraph = function drawGraph() {
    ...
    if (xLabelRotation === 45 && (0, _isTruthy.default)(showBrush)) {
      onError((0, _translation.t)('You cannot use 45° tick layout along with the time range filter'));
      return null;
    }

    const canShowBrush = (0, _isTruthy.default)(showBrush) || showBrush === 'auto' && maxHeight >= MIN_HEIGHT_FOR_BRUSH && xTicksLayout !== '45°';
    const numberFormatter = (0, _numberFormat.getNumberFormatter)(numberFormat);

    switch (vizType) {
      case 'line':
        if (canShowBrush) {
          chart = _nvd.default.models.lineWithFocusChart();
          ...
          chart = _nvd.default.models.lineChart();
        }
      ...
      case 'dual_line':
      case 'line_multi':
        chart = _nvd.default.models.multiChart();
        chart.interpolate(lineInterpolation);
        break;

      case 'bar':
        chart = _nvd.default.models.multiBarChart().showControls(showControls).groupSpacing(0.1);
      ...
      case 'dist_bar':
        chart = _nvd.default.models.multiBarChart().showControls(showControls).reduceXTicks(reduceXTicks).groupSpacing(0.1); // Distance between each group of bars.
      ...
      case 'pie':
        chart = _nvd.default.models.pieChart();
        colorKey = 'x';
      ...
      case 'column':
        chart = _nvd.default.models.multiBarChart().reduceXTicks(false);
        break;

      case 'compare':
        chart = _nvd.default.models.cumulativeLineChart();
        chart.xScale(_d.default.time.scale.utc());
      ...
      case 'bubble':
        chart = _nvd.default.models.scatterChart();
      ...
      case 'area':
        chart = _nvd.default.models.stackedAreaChart();
      ...
      case 'box_plot':
        colorKey = 'label';
        chart = _nvd.default.models.boxPlotChart();
        chart.x(d => d.label);
      ...
      case 'bullet':
        chart = _nvd.default.models.bulletChart();
        break;
    ...
    if (isTimeSeries) {
      xAxisFormatter = (0, _timeFormat.getTimeFormatter)(xAxisFormat); // In tooltips, always use the verbose time format

      chart.interactiveLayer.tooltip.headerFormatter(_timeFormat.smartDateVerboseFormatter);
    } else {
    ...
    if (chart.yAxis && chart.yAxis.tickFormat) {
      if (contribution || comparisonType === 'percentage') {
        ...
        yAxisFormatter = (0, _numberFormat.getNumberFormatter)(_numberFormat.NumberFormats.PERCENT_1_POINT);
      }
    ...
    if (isVizTypes(['line', 'area']) && useRichTooltip) {
      ...
      if (vizType === 'line') {
        chart.interactiveLayer.tooltip.contentGenerator(d => (0, _utils.generateRichLineTooltipContent)(d, _timeFormat.smartDateVerboseFormatter, yAxisFormatter));
      } else if (areaStackedStyle !== 'expand') {
        // area chart
        chart.interactiveLayer.tooltip.contentGenerator(d => (0, _utils.generateAreaChartTooltipContent)(d, _timeFormat.smartDateVerboseFormatter, yAxisFormatter));
      }
    }

    if (isVizTypes(['dual_line', 'line_multi'])) {
      const yAxisFormatter1 = (0, _numberFormat.getNumberFormatter)(yAxisFormat);
      const yAxisFormatter2 = (0, _numberFormat.getNumberFormatter)(yAxis2Format);
    ...
    if (chart.yAxis !== undefined || chart.yAxis2 !== undefined) {
      ...
      svg.datum(data).transition().duration(500) // .attr('transform', `translate(${width},0)`)
      .attr('width', width).attr('height', height).attr('x', "-" + width).call(chart); // On scroll, hide (not remove) tooltips so they can reappear on hover.
      ...
      if (isTimeSeries && activeAnnotationLayers.length > 0) {
        // Formula annotations
        const formulas = activeAnnotationLayers.filter(a => a.annotationType === _AnnotationTypes.default.FORMULA).map(a => _extends({}, a, {
          formula: _mathjs.default.parse(a.value)
        }));
        let xMax;
        let xMin;
        let xScale;
        ...
        if (annotationData) {
          ...
          activeAnnotationLayers.filter(x => x.annotationType === _AnnotationTypes.default.EVENT && annotationData && annotationData[x.name]).forEach((config, index) => {
            ...
            const tip = (0, _utils.tipFactory)(_extends({}, e, {
  ...
  _nvd.default.addGraph(drawGraph); // d3.select('.superset-legacy-chart-nvd3-bar').node().scrollBy(400,0);
